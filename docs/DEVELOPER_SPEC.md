...
| 要件 | 実装 |
|------|------|
| ボーダー・回転率スコア | `analysis/border_ev.py` + `engine.analyze_store` |
| オカルトモード OFF（既定） | `OCULT_MODE=0` → 凹み・波形比重を大幅削減 |
| ana-slo 回転列 | `anaslo/parser.py` ヘッダ「回転」 |
| 実績照合 JST 修正 | `timeutil.jst_day_bounds_utc` + `feedback.record_outcomes` |
